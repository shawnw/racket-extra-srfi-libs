...
(provide (all-from-out "base.rkt")
         ...
         (contract-out
          ...
          (s16vector-fold (-> procedure? any/c s16vector? s16vector? ... any/c))
          (s16vector-fold-right
           (-> procedure? any/c s16vector? s16vector? ... any/c))
          (s16vector-map (-> procedure? s16vector? s16vector? ... s16vector?))
          (s16vector-map! (-> procedure? s16vector? s16vector? ... s16vector?))
          (s16vector-for-each (-> procedure? s16vector? s16vector? ... void?))
          (s16vector-count
           (-> procedure? s16vector? s16vector? ... exact-nonnegative-integer?))
          (s16vector-cumulate
          ...
          (s16vector-skip-right
           (->
            ...
            ...
            (or/c exact-nonnegative-integer? #f)))
          (s16vector-any (-> procedure? s16vector? s16vector? ... any))
          (s16vector-every (-> procedure? s16vector? s16vector? ... any))
          (s16vector-partition
           (->
            (-> s16? any/c)
            s16vector?
    ...
    (let loop ((i 0) (state seed))
      (unless (= i len)
        (let-values (((value newstate) (f i state)))
          (unsafe-s16vector-set! v i value)
          (loop (+ i 1) newstate))))
    v))
...
(define (s16vector-fold kons knil vec . vecs)
  (if (null? vecs)
    (let ((len (s16vector-length vec)))
      (let loop ((r knil) (i 0))
        (if (= i len) r (loop (kons r (unsafe-s16vector-ref vec i)) (+ i 1)))))
  ...
  (if (null? vecs)
    (let ((len (s16vector-length vec)))
      (let loop ((r knil) (i (- (s16vector-length vec) 1)))
        (if (negative? i)
          r
          (loop (kons r (unsafe-s16vector-ref vec i)) (- i 1)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map s16vector-length vecs))))
...
(define (s16vector-map f vec . vecs)
  (if (null? vecs)
    (let* ((len (s16vector-length vec)) (v (make-s16vector len)))
      (let loop ((i 0))
        (unless (= i len)
          (unsafe-s16vector-set! v i (f (unsafe-s16vector-ref vec i)))
          (loop (+ i 1))))
      v)
...
(define (s16vector-map! f vec . vecs)
  (if (null? vecs)
    (let ((len (s16vector-length vec)))
      (let loop ((i 0))
        (unless (= i len)
          (unsafe-s16vector-set! vec i (f (unsafe-s16vector-ref vec i)))
          (loop (+ i 1)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map s16vector-length vecs))))
      (let loop ((i 0))
        (unless (= i len)
...
(define (s16vector-for-each f vec . vecs)
  (if (null? vecs)
    (let ((len (s16vector-length vec)))
      (let loop ((i 0))
        (unless (= i len) (f (unsafe-s16vector-ref vec i)) (loop (+ i 1)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map s16vector-length vecs))))
      (let loop ((i 0))
    ...
    (let ((len (s16vector-length vec)))
      (let loop ((i 0) (r 0))
        (cond
         ...
         ((pred (unsafe-s16vector-ref vec i)) (loop (+ i 1) (+ r 1)))
         (else (loop (+ i 1) r)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map s16vector-length vecs))))
      (let loop ((i 0) (r 0))
        (cond
...
(define (s16vector-cumulate f knil vec)
  (let* ((len (s16vector-length vec)) (v (make-s16vector len)))
    (let loop ((r knil) (i 0))
      (unless (= i len)
        (let ((next (f r (unsafe-s16vector-ref vec i))))
          (unsafe-s16vector-set! v i next)
          (loop next (+ i 1)))))
    v))
  ...
  (if (null? vecs)
    (let ((len (s16vector-length vec)))
      (let loop ((i 0))
        (cond
         ((= i len) #f)
         ((pred (unsafe-s16vector-ref vec i)) i)
         (else (loop (+ i 1))))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map s16vector-length vecs))))
      (let loop ((i 0))
        (cond
  ...
  (if (null? vecs)
    (let ((len (s16vector-length vec)))
      (let loop ((i 0))
        (cond
         ((= i len) #f)
         ((pred (unsafe-s16vector-ref vec i)))
         (else (loop (+ i 1))))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map s16vector-length vecs))))
      (let loop ((i 0))
        (cond
  ...
  (if (null? vecs)
    (let ((len (s16vector-length vec)))
      (let loop ((i 0) (last #t))
        (cond
         ((= i len) last)
         ((pred (unsafe-s16vector-ref vec i)) => (lambda (r) (loop (+ i 1) r)))
         (else #f))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map s16vector-length vecs))))
      (let loop ((i 0) (last #t))
        (cond
  ...
  (let* ((len (s16vector-length vec))
    ...
    (let loop ((i 0) (yes 0) (no cnt))
      (cond
       ((= i len) r)
       ((pred (unsafe-s16vector-ref vec i))
        (unsafe-s16vector-set! r yes (unsafe-s16vector-ref vec i))
        (loop (+ i 1) (+ yes 1) no))
       (else
        (unsafe-s16vector-set! r no (unsafe-s16vector-ref vec i))
        (loop (+ i 1) yes (+ no 1)))))))
  ...
  (let* ((len (s16vector-length vec))
    ...
    (let loop ((i 0) (j 0))
      (cond
       ((= i len) r)
       ((pred (unsafe-s16vector-ref vec i))
        (unsafe-s16vector-set! r j (unsafe-s16vector-ref vec i))
        (loop (+ i 1) (+ j 1)))
       (else (loop (+ i 1) j))))))
  ...
  (begin
    ...
    (let ((last (- (s16vector-length vec) 1)))
      (let loop ((i 0))
        (cond
         ((= i last)
          (write (unsafe-s16vector-ref vec i) port)
          (display ")" port))
         (else
          (write (unsafe-s16vector-ref vec i) port)
          (display " " port)
    ...
    (cond
     ...
     (else
      (let loop ((i 0))
        (cond
         ((= i len1) #f)
         ((< (unsafe-s16vector-ref vec1 i) (unsafe-s16vector-ref vec2 i)) #t)
         ((> (unsafe-s16vector-ref vec1 i) (unsafe-s16vector-ref vec2 i)) #f)
         (else (loop (+ i 1)))))))))
  ...
  (let ((len (min 256 (s16vector-length vec))))
    (let loop ((i 0) (r 0))
      (if (= i len)
        ...
        (loop (+ i 1) (+ r (unsafe-s16vector-ref vec i)))))))
