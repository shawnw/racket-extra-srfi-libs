...
(define (f64vector-fold kons knil vec . vecs)
  (if (null? vecs)
    (let ((len (f64vector-length vec)))
      (let loop ((r knil) (i 0))
        (if (= i len) r (loop (kons r (f64vector-ref vec i)) (+ i 1)))))
  ...
  (if (null? vecs)
    (let ((len (f64vector-length vec)))
      (let loop ((r knil) (i (- (f64vector-length vec) 1)))
        (if (negative? i) r (loop (kons r (f64vector-ref vec i)) (- i 1)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map f64vector-length vecs))))
...
(define (f64vector-for-each f vec . vecs)
  (if (null? vecs)
    (let ((len (f64vector-length vec)))
      (let loop ((i 0))
        (unless (= i len) (f (f64vector-ref vec i)) (loop (+ i 1)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map f64vector-length vecs))))
      (let loop ((i 0))
...
(define (write-f64vector vec (port (current-output-port)))
  ...
  (let ((last (- (f64vector-length vec) 1)))
    (let loop ((i 0))
      (cond
       ((= i last) (write (f64vector-ref vec i) port) (display ")" port))
       (else
    ...
    (cond
     ...
     (else
      (let loop ((i 0))
        (cond
         ((= i len1) #f)
         ((< (f64vector-ref vec1 i) (f64vector-ref vec2 i)) #t)
         ((> (f64vector-ref vec1 i) (f64vector-ref vec2 i)) #f)
         (else (loop (+ i 1)))))))))
