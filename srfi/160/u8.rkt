...
(define (u8vector-copy vec (start 0) (end (u8vector-length vec)))
  (let ((v (make-u8vector (- end start))))
    (u8vector-copy! v 0 vec start end)
    v))

(define (u8vector-copy! to at from (start 0) (end (u8vector-length from)))
  (let loop ((at at) (i start))
    (unless (= i end)
      (u8vector-set! to at (u8vector-ref from i))
      (loop (+ at 1) (+ i 1)))))

(define (u8vector-reverse-copy vec (start 0) (end (u8vector-length vec)))
  (let ((v (make-u8vector (- end start))))
...
(define (len-sum vecs)
  (if (null? vecs) 0 (+ (u8vector-length (car vecs)) (len-sum (cdr vecs)))))
...
(define (u8vector-empty? vec) (zero? (u8vector-length vec)))

(define (u8vector= vec1 vec2 . vecs)
  (and (u8dyadic-vecs=
        vec1
        0
        (u8vector-length vec1)
        vec2
        0
...
(define (u8vector-take-right vec n)
  (let ((v (make-u8vector n)) (len (u8vector-length vec)))
    (u8vector-copy! v 0 vec (- len n) len)
    v))

(define (u8vector-drop vec n)
  (let* ((len (u8vector-length vec)) (vlen (- len n)) (v (make-u8vector vlen)))
    (u8vector-copy! v 0 vec n len)
    v))

(define (u8vector-drop-right vec n)
  (let* ((len (u8vector-length vec)) (rlen (- len n)) (v (make-u8vector rlen)))
...
(define (u8vector-fold kons knil vec . vecs)
  (if (null? vecs)
    (let ((len (u8vector-length vec)))
      (let loop ((r knil) (i 0))
        (if (= i len) r (loop (kons r (u8vector-ref vec i)) (+ i 1)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map u8vector-length vecs))))
      (let loop ((r knil) (i 0))
        (if (= i len)
          r
  ...
  (if (null? vecs)
    (let ((len (u8vector-length vec)))
      (let loop ((r knil) (i (- (u8vector-length vec) 1)))
        (if (negative? i) r (loop (kons r (u8vector-ref vec i)) (- i 1)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map u8vector-length vecs))))
...
(define (u8vector-map f vec . vecs)
  (if (null? vecs)
    (let* ((len (u8vector-length vec)) (v (make-u8vector len)))
      (let loop ((i 0))
        (unless (= i len)
...
(define (u8vector-for-each f vec . vecs)
  (if (null? vecs)
    (let ((len (u8vector-length vec)))
      (let loop ((i 0))
        (unless (= i len) (f (u8vector-ref vec i)) (loop (+ i 1)))))
    (let* ((vecs (cons vec vecs)) (len (apply min (map u8vector-length vecs))))
      (let loop ((i 0))
...
(define (u8vector-fill! vec fill (start 0) (end (u8vector-length vec)))
  (unless (= start end)
    (u8vector-set! vec start fill)
    (u8vector-fill! vec fill (+ start 1) end)))

(define (u8vector-reverse! vec (start 0) (end (u8vector-length vec)))
...
(define (reverse-u8vector->list vec (start 0) (end (u8vector-length vec)))
  (let loop ((i start) (r '()))
...
(define (u8vector->vector vec (start 0) (end (u8vector-length vec)))
...
(define (make-u8vector-generator vec (start 0) (end (u8vector-length vec)))
  (lambda ()
    (if (>= start end)
      eof
...
(define (write-u8vector vec (port (current-output-port)))
  ...
  (let ((last (- (u8vector-length vec) 1)))
    (let loop ((i 0))
      (cond
       ((= i last) (write (u8vector-ref vec i) port) (display ")" port))
       (else
...
(define (u8vector< vec1 vec2)
  (let ((len1 (u8vector-length vec1)) (len2 (u8vector-length vec2)))
    (cond
     ((< len1 len2) #t)
     ((> len1 len2) #f)
     (else
      (let loop ((i 0))
        (cond
         ((= i len1) #f)
         ((< (u8vector-ref vec1 i) (u8vector-ref vec2 i)) #t)
         ((> (u8vector-ref vec1 i) (u8vector-ref vec2 i)) #f)
         (else (loop (+ i 1)))))))))
