...
;;; SRFI-180 reference implementation adapted to Racket by Shawn Wagner

(require racket/contract racket/math srfi/13 "133.rkt" "145.rkt" "158.rkt")

(define port-or-generator/c (or/c input-port? (-> (or/c char? eof-object?))))
(define port-or-accumulator/c (or/c output-port? (-> (or/c char? string?) any/c)))

(define json/c (flat-rec-contract json boolean? real? string? (vectorof json #:flat? #t) (listof (cons/c symbol? json)) 'null))

(provide
 (contract-out
  [json-number-of-character-limit (parameter/c (or/c integer? (=/c +inf.0)))]
  [json-nesting-depth-limit (parameter/c (or/c integer? (=/c +inf.0)))]
  [json-null? predicate/c]
  (rename exn:fail:json? json-error? predicate/c)
  [json-error-reason (-> exn:fail:json? string?)]
  [json-fold (->* ((-> any/c any/c any/c) (-> any/c any/c) (-> any/c any/c) (-> any/c any/c) (-> any/c any/c) any/c)
                  (port-or-generator/c)
                  any/c)]
  [json-generator (->* () (port-or-generator/c) (-> (or/c symbol? boolean? real? string? eof-object?)))]
  [json-read (->* () (port-or-generator/c) json/c)]
  [json-lines-read (->* () (port-or-generator/c) (-> (or/c json/c eof-object?)))]
  [json-sequence-read (->* () (port-or-generator/c) (-> (or/c json/c eof-object?)))]
  [json-accumulator (-> port-or-accumulator/c (-> any/c void?))]
  [json-write (->* (json/c) (port-or-accumulator/c) void?)]
  ))
...
(struct exn:fail:json exn:fail ()
  #:extra-constructor-name make-exn:fail:json
  #:transparent)
(define (make-json-error reason)
  (make-exn:fail:json reason (current-continuation-marks)))
(define (json-error-reason jerror)
  (exn-message jerror))
...
(define (json-whitespace? char)
  (case char
    ((#\u20 ; Space
      #\u09 ; Horizontal tab
      #\u0A ; Line feed or New line
      #\u0D
      #\u1E  ;; Record Separator
      )
     #t)
    (else #f)))
...
(define (port->generator port)
  (let ((count 0)
    ...
    (lambda ()
      (let ((out (with-handlers
                     ([exn:fail:read? (lambda (ex) (raise (make-json-error "Read error!")))])
                   (read-char port))))
        (if (= count limit)
...
(define (%json-tokens generator)
              ...
              (case char
                ...
                ((#\b) (loop (generator) (cons #\backspace
                                                    (append chars-unescaped
                                                            out))))
                ((#\f) (loop (generator) (cons #\u0C
                                                    (append chars-unescaped
                                                            out))))
                ((#\n) (loop (generator) (cons #\newline
                                                    (append chars-unescaped
                                                            out))))
                ((#\r) (loop (generator) (cons #\u0D
                                                    (append chars-unescaped
                                                            out))))
  ...
  (define (valid-number? string)
    ...
    (regexp-match-exact? #px"-?(?:0|[1-9][0-9]*)(?:\\.[0-9]+)?(?:[eE][+-]?[0-9]+)?" string))
  ...
  (let ((char (generator)))
    (if (eof-object? char)
        char  ;; return an empty generator
        (begin
...
(define (json-generator [port-or-generator (current-input-port)])
  (%json-generator (json-tokens (if (input-port? port-or-generator) (port->generator port-or-generator) port-or-generator))))
...
(define (json-fold proc array-start array-end object-start object-end seed [port-or-generator (current-input-port)])
...
(define (json-read [port-or-generator (current-input-port)])
  (%json-read port-or-generator))

;; json-lines-read

(define (json-lines-read [port-or-generator (current-input-port)])
  (lambda ()
    (json-read port-or-generator)))

;; json-sequence-read

(define (json-sequence-read [port-or-generator (current-input-port)])
  (lambda ()
    (let loop ()
      (with-handlers ([exn:fail:json? (lambda (ex) (loop))])
        (json-read port-or-generator)))))
...
(define (json-accumulator port-or-accumulator)
  (if (procedure? port-or-accumulator)
      ...
      (%json-accumulator (port->accumulator port-or-accumulator))))
                  

(define (json-write obj [port-or-accumulator (current-output-port)])
  (if (procedure? port-or-accumulator)
