...
;;; Also consider rewriting in Typed Racket.

(require racket/require racket/contract (only-in racket/math nan?)
...
(define nan/c (and/c real? nan?))
(provide
 (contract-out
  [make-nan (->* (boolean? boolean? exact-positive-integer?) (real?) nan/c)]
  [nan-negative? (-> nan/c boolean?)]
  [nan-quiet? (-> nan/c boolean?)]
  [nan-payload (-> nan/c exact-positive-integer?)]
...
(define (make-nan negative? quiet? payload [float 0.0])
  (unless (double-flonum? float)
    (raise-argument-error 'make-nan "double-flonum?" float))
  (unless (and (>= payload 1)
               (<= payload (- (arithmetic-shift 1 51) 1)))
    (raise-argument-error 'make-nan "(integer-in 1 (- (arithmetic-shift 1 51) 1))" payload))
  (unless (thread-cell-ref byte-buffer)
...
(define (nan-payload nan)
  ...
  (let ([bv (thread-cell-ref byte-buffer)])
    (real->floating-point-bytes nan 8 #t bv)
    (bytes-set! bv 8 0)
    (bytes-set! bv 9 (fxand (bytes-ref bv 1) #b111))
...
(module+ test
  ...
  (check-equal? (nan-payload q+NaN) 1)
